...
const BUNDLED_VERSION_MESSAGE = 'Using bundled PyHipster';
const LOCAL_VERSION_MESSAGE = "Switching to PyHipster installed locally in current project's node repository (node_modules)";

if (!semver.satisfies(currentNodeVersion, minimumNodeVersion)) {
  logger.fatal(
    `You are running Node version ${currentNodeVersion}\nPyHipster requires Node version ${minimumNodeVersion}\nPlease update your version of Node.`
  );
}
...
function requireCLI(preferLocal) {
  ...
  if (preferLocal) {
    try {
      const localCLI = require.resolve(path.join(process.cwd(), 'node_modules', 'generator-pyhipster', 'cli', 'cli.js'));
